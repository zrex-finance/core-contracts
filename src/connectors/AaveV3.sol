...
contract AaveV3Connector {
    ...
    function borrowOnBehalfOf(address token, uint256 amount, uint256 rateMode, address onBehalfOf) external payable {
        IAave aave = IAave(aaveProvider.getPool());
    ...
    function paybackOnBehalfOf(address token, uint256 amount, uint256 rateMode, address onBehalfOf) external payable {
        ...
        amount = amount == type(uint256).max ? getOnBehalfOfPaybackBalance(token, rateMode, onBehalfOf) : amount;
    ...
    function delegateBorrow(address token, uint256 amount, uint256 rateMode, address delegateTo) external payable {
