...
import { ERC20, PositionRouter, FlashResolver, Euler, Exchanges } from "../typechain-types";
...
describe("Position euler", async () => {
  ...
  beforeEach(async () => {
    ...
    const eulerResolverFactory = await ethers.getContractFactory("Euler");
    eulerResolver = await eulerResolverFactory.deploy();
    await eulerResolver.deployed();
  ...
  it.skip("open and close", async () => {
    ...
    const deposit = eulerResolver.interface.encodeFunctionData("deposit", [SUB_ACCOUNT_ID, position.collateral, MAX_UINT, true]);
    const borrow = eulerResolver.interface.encodeFunctionData("borrow", [SUB_ACCOUNT_ID, position.debt, _amts[0].add(bestFee_)])
    ...
    const calldataOpen = encoder.encode(
      ...
      [
        ...
        [eulerResolver.address, eulerResolver.address],
        [deposit, borrow],
    ...
    const collateralAmount = await eulerResolver.callStatic.collateralBalanceOf(
    ...
    const borrowAmount = await eulerResolver.callStatic.borrowBalanceOf(
      position.debt,
      positionRouter.address,
      SUB_ACCOUNT_ID
    );
    ...
    const repay = eulerResolver.interface.encodeFunctionData("repay", [SUB_ACCOUNT_ID, position.debt, MAX_UINT])
    const withdraw = eulerResolver.interface.encodeFunctionData("withdraw", [SUB_ACCOUNT_ID, position.collateral, MAX_UINT])
