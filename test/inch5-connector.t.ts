...
describe("OneInch v5 connector", async () => {
  ...
  it("Swap ETH to USDC", async () => {
    ...
    await inchV5Connector.connect(other).swap(DAI_CONTRACT, ETH_CONTRACT_2, swapAmount, calldata as string, { value: swapAmount })

    expect(
        parseInt(ethers.utils.formatEther(
            (await daiContract.callStatic.balanceOf(inchV5Connector.address)).toString()
        ))
    ).to.greaterThan(0);
  });
});