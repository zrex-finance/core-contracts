...
describe.skip("Open and close (inchv5 - aave2)", async () => {
  ...
  beforeEach(async () => {
    ...
    await addressesProvider.connect(owner)
      .setAddress(ethers.utils.formatBytes32String('ACL_ADMIN'), owner.address);
    ...
    await addressesProvider.connect(owner)
      .setAddress(ethers.utils.formatBytes32String('ACCOUNT'), account.address);

    await addressesProvider.connect(owner)
      .setAddress(ethers.utils.formatBytes32String('TREASURY'), owner.address);

    await addressesProvider.connect(owner)
      .setAddress(ethers.utils.formatBytes32String('ACCOUNT_PROXY'), proxy.address);

    await addressesProvider.connect(owner)
      .setAddress(ethers.utils.formatBytes32String('FLASHLOAN_AGGREGATOR'), flashAggregator.address);   

    openPositionCallback = account.interface.getSighash(
      "openPositionCallback(string[],bytes[],bytes[],uint256)"
    );

    closePositionCallback = account.interface.getSighash(
      "closePositionCallback(string[],bytes[],bytes[],uint256)"
    );
  });
  ...
  it("swap and open - close position", async () => {
    ...
    const calldataOpen = encoder.encode(
      ...
      [
        ...
        ['OneInchV5','AaveV2','AaveV2'],
        [positionSwapCalldata, deposit, borrow],
        [key],
      ]
    )
    ...
    const payback = aveV2ConnectorContract.interface.encodeFunctionData("payback", [position.debt, MAX_UINT, RATE_TYPE_AAVE])
    const withdraw = aveV2ConnectorContract.interface.encodeFunctionData("withdraw", [position.collateral, collateralAmount])

    const calldataClose = encoder.encode(
      ["bytes4", "string[]", "bytes[]", "bytes[]",],
      [
        closePositionCallback,
        ['AaveV2','AaveV2', 'OneInchV5'],
        [payback, withdraw, positionCloseSwapCalldata],
        [key],
      ]
    )
    ...
    const { bestRoutes: closeRoutes } = await flashResolverContract.callStatic.getData([position.debt], [flashloanAmount]);
